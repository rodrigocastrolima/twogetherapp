...
// Run with: node test_firebase_functions.cjs
//
...
async function testPingFunction() {
  ...
  try {
    // Create a callable function for the ping endpoint
    console.log(`Calling ping function in ${REGION}...`);
    const startTime = Date.now();
    
    // Using v1 style API since v2 is causing issues
    const result = await admin.app().functions(REGION).httpsCallable('ping')();
...
async function testCreateUserFunction(email, password, displayName, salesforceId) {
  ...
  try {
    ...
    // Using v1 style API since v2 is causing issues
    const result = await admin.app().functions(REGION).httpsCallable('createUser')(data);
